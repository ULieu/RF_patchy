...
!------------------------------------------------------------------------
    ...
    common/amax/amax !max of absolute value of Aaniso function
    common/ncode/ncode 
...
!----------------------------------------------------
    ...
    do i=1,npm*3
        ...
        !!---testing delta_x is too large or not------------------ 
        !if (abs(anint(xpa(i)/bl)) >= 2) then        
        !open(11,file='test.dat',access='append',status='unknown')
        !write(*,*)"testing delta_x is too large: TRUE"
        !write(11,*)'delta_x is too large'
        !stop !istep=istepmax !this is to stop the program        
        !close(11)
        !endif
        !!--------------------------------------------------------        
        !xpa(i)= xpa(i)-anint(xpa(i)/bl)*bl !PBC adjustment if particle moves out of the box         
    enddo
    ...
    do ipm=1,npm            !force z=0 !just to confirm, may check the real force later ON PLANE
        xp(ipm,3)=0.d0      
    enddo
...
!-----------------------------------------------    
    subroutine initia_file
    use para  
    implicit double precision (a-h,o-z)
    
    
        
!---read info on pos and ORI0
    xp=0.d0; p0=0.d0; p1=0.d0; p2=0.d0
    open(466,file='01LastPos_previous.dat',status='old')
    open(467,file='01LastOri0_previous.dat',status='old')
    open(468,file='01LastOri1_previous.dat',status='old')
    open(469,file='01LastOri2_previous.dat',status='old')
    do ipm=1,npm
        read(466,*)xp(ipm,1:2)! the file is [xp(1:2)]
        read(467,*)p0(ipm,1:3)
        read(468,*)p1(ipm,1:3)
        read(469,*)p2(ipm,1:3)
        !write(*,*)xp(ipm,1:3)
    enddo
    close(466); close(467); close(468); close(469);  
    end subroutine initia_file
